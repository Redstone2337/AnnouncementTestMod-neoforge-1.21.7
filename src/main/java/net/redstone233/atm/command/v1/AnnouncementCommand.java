...
public class AnnouncementCommand {
    ...
    public static LiteralArgumentBuilder<CommandSourceStack> register() {
        return Commands.literal("atm-v1")
        ...
        if (isDisplay) {
            ...
            if (player != null) {
                if (player.level().isClientSide) {
                    // 使用 V1ConfigManager 创建配置并打开屏幕
                    Minecraft.getInstance().setScreen(new AnnouncementScreen(ConfigManager.createAnnouncementConfig()));
                }
            }
    ...
    private static int executeSetMainTitle(CommandSourceStack source, Component message) {
        try {
            String finalCurrentTitle = message.getString();
            config.mainTitle = finalCurrentTitle;
    ...
    private static int executeEmptyMainTitle(CommandSourceStack source, boolean isEmpty) {
        if (isEmpty) {
            try {
                String currentTitle = "";
                config.mainTitle = currentTitle;
                source.sendSuccess(() -> Component.translatable("commands.atm.settings.maintitle.empty.status", currentTitle), true);
                return 1;
            } catch (Exception e) {
    ...
    private static int executeSetSubTitle(CommandSourceStack source, Component message) {
        try {
            String currentSubTitle = message.getString();
            config.subTitle = currentSubTitle;
            source.sendSuccess(() -> Component.translatable("commands.atm.settings.subtitle.set.success", currentSubTitle), true);
            return 1;
        } catch (Exception e) {
    ...
    private static int executeEmptySubTitle(CommandSourceStack source, boolean isEmpty) {
        if (isEmpty) {
            try {
                String currentTitle = "";
                config.subTitle = currentTitle;

                source.sendSuccess(() -> Component.translatable("commands.atm.settings.subtitle.empty.status", currentTitle), true);
                return 1;
            } catch (Exception e) {
