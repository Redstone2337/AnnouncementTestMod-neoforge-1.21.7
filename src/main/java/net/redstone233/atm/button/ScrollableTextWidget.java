...
public class ScrollableTextWidget extends AbstractWidget {
    ...
    public void updateTextLines() {
        ...
        try {
            if (getMessage() == null) {
                ...
                // 直接按换行符切分，保留 Component 对象本身（已含颜色）
                getMessage().visit((style, text) -> {
                    for (String line : text.split("\n")) {
                        textLines.add(Component.literal(line).setStyle(style));
                    }
            ...
            if (AnnouncementTestModClient.DEBUG_MODE) {
                AnnouncementTestMod.LOGGER.info("处理了 {} 行文本，总高度: {}", textLines.size(), totalHeight);
                for (int i = 0; i < textLines.size(); i++) {
                    AnnouncementTestMod.LOGGER.info("行 {}: {}", i, textLines.get(i).getString());
                }
            }

            totalHeight = textLines.size() * font.lineHeight;
        } catch (Exception e) {
            AnnouncementTestMod.LOGGER.error("文本处理失败", e);
    ...
    private Component parseFormattingCodes(String text) {
        ...
        net.minecraft.ChatFormatting currentFormatting = null;
        ...
        for (int i = 0; i < text.length(); i++) {
            ...
            if (c == '§' && i + 1 < text.length()) {
                ...
                net.minecraft.ChatFormatting formatting = net.minecraft.ChatFormatting.getByCode(codeChar);
    ...
    protected void renderWidget(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick) {
        ...
        for (Component line : textLines) {
            if (yOffset + font.lineHeight >= getY() && yOffset <= getY() + height) {
                // 使用Component对象自身的颜色，而不是固定颜色
                int textColor = line.getStyle().getColor() != null ?
                        line.getStyle().getColor().getValue() : color;
    ...
    protected void updateWidgetNarration(NarrationElementOutput narrationElementOutput) {
        narrationElementOutput.add(NarratedElementType.TITLE, Component.translatable("narration.scrollable_text", this.getMessage()));
        narrationElementOutput.add(NarratedElementType.USAGE, Component.translatable("narration.scrollable_text.usage"));
    }
